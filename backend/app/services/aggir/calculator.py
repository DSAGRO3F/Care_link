...
class Adverbes:
    ...
    def to_letter(self) -> str:
        ...
        if all([self.spontanement, self.totalement, self.correctement, self.habituellement]):
            return "A"
        if not any([self.spontanement, self.totalement, self.correctement, self.habituellement]):
            return "C"
        return "B"
...
class AggirCalculator:
    ...
    def combiner_sous_variables_coherence(communication: str, comportement: str) -> str:
        ...
        if communication == "A" and comportement == "A":
            return "A"
        if "C" in [communication, comportement]:
            return "C"
        return "B"
    ...
    def combiner_sous_variables_habillage(haut: str, moyen: str, bas: str) -> str:
        ...
        if haut == "A" and moyen == "A" and bas == "A":
            return "A"
        if haut == "C" and moyen == "C" and bas == "C":
            return "C"
        return "B"
    ...
    @staticmethod
    def determiner_gir_depuis_score(score: int, groupe_name: str) -> int | None:
        """
    ...
    def calculer_gir(self, evaluations: dict[Variable, Adverbes]) -> tuple[int, dict[str, Any]]:
        ...
        # Cohérence
        if (
            Variable.COMMUNICATION in lettres_sous_variables
            and Variable.COMPORTEMENT in lettres_sous_variables
        ):
        ...
        # Orientation
        if (
            Variable.ORIENTATION_TEMPS in lettres_sous_variables
            and Variable.ORIENTATION_ESPACE in lettres_sous_variables
        ):
        ...
        # Toilette
        if (
            Variable.TOILETTE_HAUT in lettres_sous_variables
            and Variable.TOILETTE_BAS in lettres_sous_variables
        ):
        ...
        # Habillage
        if all(
            v in lettres_sous_variables
            for v in [Variable.HABILLAGE_HAUT, Variable.HABILLAGE_MOYEN, Variable.HABILLAGE_BAS]
        ):
        ...
        # Alimentation
        if (
            Variable.SE_SERVIR in lettres_sous_variables
            and Variable.MANGER in lettres_sous_variables
        ):
            lettres_principales[Variable.ALIMENTATION] = self.combiner_sous_variables_alimentation(
                lettres_sous_variables[Variable.SE_SERVIR], lettres_sous_variables[Variable.MANGER]
            )

        # Élimination
        if (
            Variable.ELIMINATION_URINAIRE in lettres_sous_variables
            and Variable.ELIMINATION_FECALE in lettres_sous_variables
        ):
        ...
        for var in [
            Variable.TRANSFERTS,
            Variable.DEPLACEMENTS_INTERNES,
            Variable.DEPLACEMENTS_EXTERNES,
            Variable.ALERTER,
        ]:
