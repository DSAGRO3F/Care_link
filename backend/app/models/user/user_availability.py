...
from sqlalchemy import Boolean, CheckConstraint, Date, ForeignKey, Integer, Text, Time
...
class UserAvailability(TimestampMixin, Base):
    ...
    day_of_week: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="Jour de la semaine (1=Lundi ... 7=Dimanche)"
    )

    # === Créneau horaire ===
    start_time: Mapped[time] = mapped_column(Time, nullable=False, comment="Heure de début")

    end_time: Mapped[time] = mapped_column(Time, nullable=False, comment="Heure de fin")
    ...
    valid_from: Mapped[date | None] = mapped_column(
        Date, nullable=True, comment="Date de début de validité"
    )

    valid_until: Mapped[date | None] = mapped_column(
        Date, nullable=True, comment="Date de fin de validité (NULL = indéfiniment)"
    )

    # === Capacité ===
    max_patients: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="Nombre max de patients sur ce créneau"
    )

    # === Notes ===
    notes: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="Notes (ex: 'sauf jours fériés')"
    )

    # === Statut ===
    is_active: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=True, comment="Disponibilité active"
    )

    # === Relations ===
    user: Mapped[User] = relationship("User", back_populates="availabilities")

    entity: Mapped[Entity | None] = relationship("Entity", back_populates="user_availabilities")

    tenant: Mapped[Tenant] = relationship(
        "Tenant", doc="Tenant propriétaire de cette disponibilité"
    )
