...
class PatientAccess(Base):
    ...
    granted_by: Mapped[int] = mapped_column(
        ...
        info={"description": "Administrateur ou coordinateur"},
    )
    ...
    expires_at: Mapped[datetime | None] = mapped_column(
        ...
        info={"description": "Expiration automatique, NULL si permanent"},
    )

    revoked_at: Mapped[datetime | None] = mapped_column(
        nullable=True,
        doc="Date de révocation de l'accès (None = actif)",
        info={"description": "NULL si l'accès est toujours actif"},
    )
    ...
    revoked_by: Mapped[int | None] = mapped_column(
        ...
        info={"description": "Qui a révoqué l'accès"},
    )

    # === Relations ===

    patient: Mapped["Patient"] = relationship(
        "Patient", back_populates="access_records", doc="Patient concerné"
    )

    user: Mapped["User"] = relationship(
        "User", foreign_keys=[user_id], doc="Utilisateur ayant accès"
    )

    granter: Mapped["User"] = relationship(
        "User", foreign_keys=[granted_by], doc="Utilisateur ayant accordé l'accès"
    )

    revoker: Mapped["User | None"] = relationship(
        "User", foreign_keys=[revoked_by], doc="Utilisateur ayant révoqué l'accès"
    )
