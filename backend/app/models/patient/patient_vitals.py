...
from app.models.enums import DeviceType, VitalSource, VitalStatus, VitalType
...
class PatientThreshold(TimestampMixin, Base):
    ...
    patient: Mapped["Patient"] = relationship(
        "Patient", back_populates="thresholds", doc="Patient concerné"
    )
...
class PatientVitals(TimestampMixin, Base):
    ...
    vital_type: Mapped[str] = mapped_column(
        ...
        nullable=False,
        doc="Type de constante vitale",
        info={"description": "Type de mesure", "enum": [e.value for e in VitalType]},
    )
    ...
    source: Mapped[str] = mapped_column(
        ...
        info={"description": "Origine de la mesure", "enum": [e.value for e in VitalSource]},
    )
    ...
    patient: Mapped["Patient"] = relationship(
        "Patient", back_populates="vitals", doc="Patient concerné"
    )

    device: Mapped["PatientDevice | None"] = relationship(
        "PatientDevice", doc="Device source de la mesure"
    )

    recorder: Mapped["User | None"] = relationship(
        "User", doc="Professionnel ayant enregistré la mesure"
    )
    ...
    def is_abnormal(self) -> bool:
        ...
        return self.status in [
            VitalStatus.LOW.value,
            VitalStatus.HIGH.value,
            VitalStatus.CRITICAL.value,
        ]
...
class PatientDevice(TimestampMixin, Base):
    ...
    device_type: Mapped[str] = mapped_column(
        ...
        nullable=False,
        doc="Type de device",
        info={"description": "Marque/type du device", "enum": [e.value for e in DeviceType]},
    )
    ...
    patient: Mapped["Patient"] = relationship(
        "Patient", back_populates="devices", doc="Patient propriétaire du device"
    )
