...
from app.models.enums import IntegrationType, TenantStatus, TenantType
...
class Tenant(Base, TimestampMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint("parent_tenant_id != id", name="ck_tenant_no_self_parent"),
    ...
    code: Mapped[str] = mapped_column(
        String(50), unique=True, nullable=False, comment="Code unique du tenant (ex: GCSMS-BV-IDF)"
    )
    name: Mapped[str] = mapped_column(String(255), nullable=False, comment="Nom commercial")
    legal_name: Mapped[str | None] = mapped_column(String(255), comment="Raison sociale officielle")
    siret: Mapped[str | None] = mapped_column(String(14), comment="Numéro SIRET (France)")
    ...
    # ========================
    parent_tenant_id: Mapped[int | None] = mapped_column(
    ...
    federation_date: Mapped[date | None] = mapped_column(
        Date, nullable=True, comment="Date de rattachement au groupement"
    )
    ...
    contact_email: Mapped[str] = mapped_column(
        String(255), nullable=False, comment="Email du contact principal"
    )
    contact_phone: Mapped[str | None] = mapped_column(
        String(20), comment="Téléphone du contact principal"
    )
    billing_email: Mapped[str | None] = mapped_column(
        String(255), comment="Email pour la facturation (si différent)"
    )
    ...
    country_id: Mapped[int | None] = mapped_column(
        ForeignKey("countries.id"), comment="Pays du tenant"
    )
    ...
    encryption_key_id: Mapped[str] = mapped_column(
        String(100), nullable=False, comment="Référence vers la clé AES dans le vault"
    )
    timezone: Mapped[str] = mapped_column(String(50), default="Europe/Paris", nullable=False)
    locale: Mapped[str] = mapped_column(String(10), default="fr_FR", nullable=False)

    # ========================
    # Limites et quotas
    # ========================
    max_patients: Mapped[int | None] = mapped_column(
        Integer, comment="Limite contractuelle de patients (NULL = illimité)"
    )
    max_users: Mapped[int | None] = mapped_column(
        Integer, comment="Limite contractuelle d'utilisateurs (NULL = illimité)"
    )
    max_storage_gb: Mapped[int] = mapped_column(
        Integer, default=50, nullable=False, comment="Quota de stockage en Go"
    )
    ...
    activated_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), comment="Date de mise en service"
    )
    terminated_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True), comment="Date de résiliation"
    )
    ...
    entities: Mapped[list["Entity"]] = relationship(
        "Entity", back_populates="tenant", lazy="selectin"
    )

    users: Mapped[list["User"]] = relationship("User", back_populates="tenant", lazy="selectin")

    patients: Mapped[list["Patient"]] = relationship(
        "Patient", back_populates="tenant", lazy="selectin"
    )
