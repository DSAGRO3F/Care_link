    ...
    try:
        ...
        with get_db_for_super_admin() as db:
            try:
                ...
                db.execute(
                    text(
                        f"DELETE FROM patients WHERE id IN ({data['patient1_id']}, {data['patient2_id']}, {data['patient3_id']})"
                    )
                )
                db.execute(
                    text(
                        f"DELETE FROM entities WHERE id IN ({data['entity1_id']}, {data['entity2_id']})"
                    )
                )
                db.execute(
                    text(
                        f"DELETE FROM tenants WHERE id IN ({data['tenant1_id']}, {data['tenant2_id']})"
                    )
                )
                db.commit()
...
class TestRLSBasicIsolation:
    ...
    def test_tenant1_sees_only_own_patients(self):
        ...
        with create_test_data() as data, get_db_for_tenant(data["tenant1_id"]) as db:
    ...
    def test_tenant2_sees_only_own_patients(self):
        ...
        with create_test_data() as data, get_db_for_tenant(data["tenant2_id"]) as db:
            patients = db.query(Patient).all()
            patient_ids = [p.id for p in patients]
    ...
    def test_super_admin_sees_all_patients(self):
        ...
        with create_test_data() as data, get_db_for_super_admin() as db:
            patients = db.query(Patient).all()
            patient_ids = [p.id for p in patients]
...
class TestRLSUpdateIsolation:
    ...
    def test_cannot_update_patient_in_other_tenant(self):
        ...
        with create_test_data() as data, get_db_for_tenant(data["tenant1_id"]) as db:
            ...
            result = (
                db.query(Patient)
                .filter(Patient.id == data["patient2_id"])
                .update({"first_name_encrypted": "Hacked"})
            )
    ...
    def test_can_update_patient_in_own_tenant(self):
        ...
        with create_test_data() as data, get_db_for_tenant(data["tenant1_id"]) as db:
            result = (
                db.query(Patient)
                .filter(Patient.id == data["patient1_id"])
                .update({"first_name_encrypted": "Updated"})
            )
        ...
        with create_test_data() as data:
            ...
            try:
                ...
                patients = (
                    ...
                    .filter(
                        Patient.id.in_(
                            [data["patient1_id"], data["patient2_id"], data["patient3_id"]]
                        )
                    )
                    .all()
                )
