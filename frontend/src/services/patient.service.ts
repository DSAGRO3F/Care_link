...
export const patientService = {
  ...
  async getEvaluations(
    ...
    const response = await api.get<PatientEvaluationList>(`/patients/${patientId}/evaluations`, {
      params,
    });
    return response.data;
  },
  ...
  async getEvaluation(patientId: number, evaluationId: number): Promise<PatientEvaluationResponse> {
  ...
  async getVitals(
    ...
    const response = await api.get<PatientVitalsList>(`/patients/${patientId}/vitals`, { params });
    return response.data;
  },

  /** Dernière constante d'un type */
  async getLatestVital(patientId: number, vitalType: string): Promise<PatientVitalsResponse> {
  ...
  /** Créer une mesure */
  async createVital(patientId: number, data: PatientVitalsCreate): Promise<PatientVitalsResponse> {
    const response = await api.post<PatientVitalsResponse>(`/patients/${patientId}/vitals`, data);
    return response.data;
  },
};
