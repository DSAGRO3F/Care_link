  ...
  >
    ...
    <div class="flex items-center h-16 px-4 border-b border-neutral-200">
      ...
      <div class="flex items-center gap-3">
        <div
          class="w-10 h-10 rounded-lg bg-primary-600 text-white flex items-center justify-center"
        >
          <i class="pi pi-heart text-xl"></i>
        </div>
        <span v-if="!collapsed" class="font-bold text-lg text-neutral-900"> CareLink </span>
      </div>
      ...
      <div v-for="section in menuItems" :key="section.label">
        ...
        <ul class="space-y-1">
          <li v-for="item in section.children" :key="item.to">
            ...
            >
              <i
                :class="[item.icon, isActive(item.to) ? 'text-primary-600' : 'text-neutral-400']"
                class="pi text-lg"
              ></i>
