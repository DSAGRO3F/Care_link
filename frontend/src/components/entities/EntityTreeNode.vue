...
const colors = computed(() =>
  props.dark ? EntityTypeDarkColors[props.node.entity_type] : EntityTypeColors[props.node.entity_type]
)
...
<template>
  <div class="entity-tree-node">
    ...
    <div
      ...
      :class="[
        isSelected
          ? (dark ? 'bg-teal-500/15 ring-1 ring-teal-500/40' : 'root-card-selected')
          : (dark ? 'bg-slate-800 ring-1 ring-slate-700 hover:ring-slate-600' : 'root-card'),
      ]"
    ...
    >
      ...
      <div class="p-5 pt-6">
        <div class="flex items-start justify-between">
          ...
          >
            ...
            >
              <i class="pi text-sm" :class="isExpanded ? 'pi-chevron-up' : 'pi-chevron-down'"></i>
            </button>
            <button
              class="w-8 h-8 rounded-lg flex items-center justify-center transition-colors"
              :class="dark ? 'text-emerald-400/70 hover:text-emerald-300 hover:bg-emerald-500/20' : 'text-emerald-500/70 hover:text-emerald-600 hover:bg-emerald-50'"
      ...
      <div
        class="info-grid border-t"
        :class="dark ? 'border-slate-700' : 'border-slate-100'"
      >
        <!-- 1. Code structure (avec bouton copier) -->
        <div class="info-item">
          <div class="info-icon" :class="dark ? 'bg-amber-500/15' : 'bg-amber-50'">
            <i class="pi pi-key" :class="dark ? 'text-amber-400' : 'text-amber-600'"></i>
          </div>
          <div class="min-w-0 flex-1">
            <div class="info-label" :class="dark ? 'text-slate-500' : 'text-slate-400'">Code structure</div>
            <div class="flex items-center gap-2">
              ...
              <button
                ...
                :class="codeCopied
                  ...
                  : (dark ? 'text-slate-500 hover:text-amber-400 hover:bg-amber-500/10' : 'text-slate-400 hover:text-amber-600 hover:bg-amber-50')"
                :title="codeCopied ? 'Copié !' : 'Copier le code'"
                @click.stop="copyTenantCode"
              >
                <i class="pi text-xs" :class="codeCopied ? 'pi-check' : 'pi-copy'"></i>
              </button>
            </div>
          </div>
        </div>
        ...
        <!-- 2. Adresse -->
        <div class="info-item">
          <div class="info-icon" :class="dark ? 'bg-blue-500/15' : 'bg-blue-50'">
            <i class="pi pi-map-marker" :class="dark ? 'text-blue-400' : 'text-blue-600'"></i>
          </div>
          <div class="min-w-0">
            <div class="info-label" :class="dark ? 'text-slate-500' : 'text-slate-400'">Adresse</div>
            <div class="info-value truncate" :class="dark ? 'text-slate-200' : 'text-slate-700'">
              {{ formattedAddress || '—' }}
            </div>
          </div>
        </div>
        ...
        <!-- 3. FINESS géo -->
        <div class="info-item">
          <div class="info-icon" :class="dark ? 'bg-teal-500/15' : 'bg-teal-50'">
            <i class="pi pi-id-card" :class="dark ? 'text-teal-400' : 'text-teal-600'"></i>
          </div>
          <div class="min-w-0">
            <div class="info-label" :class="dark ? 'text-slate-500' : 'text-slate-400'">FINESS géo.</div>
            <div class="info-value font-mono truncate" :class="dark ? 'text-slate-200' : 'text-slate-700'">
              {{ node.finess_geo || '—' }}
            </div>
          </div>
        </div>
        ...
        <!-- 4. Téléphone -->
        <div class="info-item">
          <div class="info-icon" :class="dark ? 'bg-slate-700' : 'bg-slate-100'">
            <i class="pi pi-phone" :class="dark ? 'text-slate-400' : 'text-slate-500'"></i>
          </div>
          <div class="min-w-0">
            <div class="info-label" :class="dark ? 'text-slate-500' : 'text-slate-400'">Téléphone</div>
            <div class="info-value truncate" :class="dark ? 'text-slate-200' : 'text-slate-700'">
              {{ node.phone || '—' }}
            </div>
          </div>
        ...
        <!-- 5. SIRET -->
        <div class="info-item">
          <div class="info-icon" :class="dark ? 'bg-violet-500/15' : 'bg-violet-50'">
            <i class="pi pi-building" :class="dark ? 'text-violet-400' : 'text-violet-600'"></i>
          </div>
          <div class="min-w-0">
            <div class="info-label" :class="dark ? 'text-slate-500' : 'text-slate-400'">SIRET</div>
            <div class="info-value font-mono truncate" :class="dark ? 'text-slate-200' : 'text-slate-700'">
              {{ node.siret || '—' }}
            </div>
          </div>
        ...
        <div class="info-item">
          <div class="info-icon" :class="dark ? 'bg-blue-500/15' : 'bg-blue-50'">
            <i class="pi pi-envelope" :class="dark ? 'text-blue-400' : 'text-blue-600'"></i>
          </div>
          <div class="min-w-0">
            <div class="info-label" :class="dark ? 'text-slate-500' : 'text-slate-400'">Email</div>
            <div class="info-value truncate" :class="dark ? 'text-slate-200' : 'text-slate-700'">
              {{ node.email || '—' }}
            </div>
          </div>
        ...
        <div class="info-item">
          <div class="info-icon" :class="dark ? 'bg-sky-500/15' : 'bg-sky-50'">
            <i class="pi pi-sitemap" :class="dark ? 'text-sky-400' : 'text-sky-600'"></i>
          </div>
          <div class="min-w-0">
            <div class="info-label" :class="dark ? 'text-slate-500' : 'text-slate-400'">Entités rattachées</div>
            <div class="info-value font-semibold" :class="dark ? 'text-slate-200' : 'text-slate-700'">
              {{ childCount }}
            </div>
          </div>
        </div>
        ...
        <div class="info-item">
          <div class="info-icon" :class="dark ? 'bg-indigo-500/15' : 'bg-indigo-50'">
            <i class="pi pi-users" :class="dark ? 'text-indigo-400' : 'text-indigo-600'"></i>
          </div>
          <div class="min-w-0">
            <div class="info-label" :class="dark ? 'text-slate-500' : 'text-slate-400'">Professionnels</div>
            <div class="info-value font-semibold" :class="dark ? 'text-slate-200' : 'text-slate-700'">
              {{ totalUsers }}
            </div>
          </div>
    ...
    <div v-else class="flex">
      ...
      <div
        ...
        :class="[
          isSelected
            ? (dark ? 'bg-violet-500/15 border-violet-500/40' : 'bg-violet-50/60 border-violet-300 shadow-sm shadow-violet-100')
            : (dark ? 'bg-slate-800 border-slate-700 hover:border-slate-600' : 'bg-white border-zinc-200 shadow-sm hover:border-violet-200 hover:shadow-md'),
        ]"
        @click="emit('select', node)"
        @mouseenter="hovered = true"
        @mouseleave="hovered = false"
        ...
        <div class="flex items-start justify-between">
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              <h4 class="font-semibold" :class="dark ? 'text-white' : 'text-zinc-800'">
                {{ node.name }}
              </h4>

              <div class="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1">
                <span v-if="node.city" class="text-xs flex items-center gap-1" :class="dark ? 'text-slate-400' : 'text-zinc-500'">
                  <i class="pi pi-map-marker" style="font-size: 0.65rem"></i>
                  {{ node.postal_code ? node.postal_code + ' ' : '' }}{{ node.city }}
                </span>
                <span v-if="node.finess_geo" class="text-xs font-mono" :class="dark ? 'text-slate-600' : 'text-zinc-300'">
                  ET {{ node.finess_geo }}
                </span>
                <span v-if="node.siret" class="text-xs font-mono" :class="dark ? 'text-slate-600' : 'text-zinc-300'">
                  SIRET {{ node.siret }}
                </span>
              </div>

              <!-- Stats enfant -->
              <div v-if="patientsCount > 0 || usersCount > 0" class="flex items-center gap-3 mt-1.5">
                <span v-if="patientsCount > 0" class="text-[11px] px-2 py-0.5 rounded-md" :class="dark ? 'bg-emerald-500/15 text-emerald-400' : 'bg-emerald-50 text-emerald-600'">
                  {{ patientsCount }} patients
                </span>
                <span v-if="usersCount > 0" class="text-[11px] px-2 py-0.5 rounded-md" :class="dark ? 'bg-sky-500/15 text-sky-400' : 'bg-sky-50 text-sky-600'">
                  {{ usersCount }} pros
                </span>
              </div>
            </div>
          ...
          >
            <button
              ...
              @click.stop="emit('toggle', node.id)"
            >
              <i class="pi text-xs" :class="isExpanded ? 'pi-chevron-up' : 'pi-chevron-down'"></i>
            </button>
            <button
              class="w-7 h-7 rounded-md flex items-center justify-center transition-colors"
              :class="dark ? 'text-emerald-400/60 hover:text-emerald-300 hover:bg-emerald-500/20' : 'text-emerald-500/60 hover:text-emerald-600 hover:bg-emerald-50'"
