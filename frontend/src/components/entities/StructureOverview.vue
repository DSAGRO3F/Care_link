<script setup lang="ts">
  ...
  async function ensureRootEntity() {
    ...
    const rootExists = entities.value.some(
      (e) => (e.parent_id === null || e.parent_id === undefined) && e.entity_type === tenantType,
    );
  ...
  async function reParentOrphans() {
    ...
    const root = entities.value.find(
      (e) =>
        (e.parent_id === null || e.parent_id === undefined) &&
    ...
    const orphans = entities.value.filter(
      (e) => (e.parent_id === null || e.parent_id === undefined) && e.id !== root.id,
    );
    ...
    for (const orphan of orphans) {
      try {
        ...
        if (import.meta.env.DEV)
          console.warn(`[StructureOverview] Failed to reparent entity ${orphan.id}`);
      }
    }
  ...
  const typeOptions = computed(() => {
    ...
    if (canManageRoot.value) {
      return [...ROOT_ENTITY_TYPES, ...CHILD_ENTITY_TYPES].map((t) => ({
        value: t,
  ...
  function entityTypeColorClasses(type: EntityType) {
    return props.dark ? EntityTypePlatformColors[type] : EntityTypeColors[type];
  }
...
<template>
  <div class="structure-overview">
    ...
    <div v-if="!embedded" class="mb-6">
      <h2 :class="dark ? 'text-white' : 'text-zinc-800'" class="text-xl font-bold">Organisation</h2>
      <p :class="dark ? 'text-slate-400' : 'text-zinc-500'" class="text-sm mt-0.5">
    ...
    <div v-if="!embedded" class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
      ...
      >
        <div class="flex items-center gap-3">
          <div
            :class="dark ? 'bg-blue-500/20' : 'bg-blue-50'"
            class="w-9 h-9 rounded-lg flex items-center justify-center"
          >
            <i :class="dark ? 'text-blue-400' : 'text-blue-500'" class="pi pi-sitemap text-sm"></i>
          </div>
          <div>
            <p
            ...
            <p :class="dark ? 'text-slate-500' : 'text-zinc-400'" class="text-[11px]">
              Entités
              <span v-if="childCount > 0" class="opacity-70"
                >({{ childCount }} membre{{ childCount > 1 ? 's' : '' }})</span
              >
            </p>
          </div>
        </div>
      ...
      >
        <div class="flex items-center gap-3">
          <div
            :class="dark ? 'bg-emerald-500/20' : 'bg-emerald-50'"
            class="w-9 h-9 rounded-lg flex items-center justify-center"
          >
            <i
              :class="dark ? 'text-emerald-400' : 'text-emerald-500'"
              class="pi pi-users text-sm"
            ></i>
          </div>
          <div>
            <p
            ...
            <p :class="dark ? 'text-slate-500' : 'text-zinc-400'" class="text-[11px]">Patients</p>
          </div>
        </div>
      </div>
      ...
      >
        <div class="flex items-center gap-3">
          <div
            :class="dark ? 'bg-sky-500/20' : 'bg-sky-50'"
            class="w-9 h-9 rounded-lg flex items-center justify-center"
          >
            <i :class="dark ? 'text-sky-400' : 'text-sky-500'" class="pi pi-briefcase text-sm"></i>
          </div>
          <div>
            <p
              :class="dark ? 'text-white' : 'text-zinc-800'"
      ...
      >
        <div class="flex items-center gap-3">
          <div
            :class="dark ? 'bg-violet-500/20' : 'bg-violet-50'"
            class="w-9 h-9 rounded-lg flex items-center justify-center"
          >
            <i
              :class="dark ? 'text-violet-400' : 'text-violet-500'"
              class="pi pi-chart-bar text-sm"
            ></i>
          </div>
          <div>
            <p
    ...
    >
      ...
      >
        <div
          :class="[
            dark ? EntityTypePlatformColors[type].bg : EntityTypeColors[type].bg,
            dark ? EntityTypePlatformColors[type].border : EntityTypeColors[type].border,
          ]"
          class="w-2 h-2 rounded-full border"
        ></div>
        <span :class="dark ? 'text-slate-400' : 'text-zinc-500'" class="text-xs">
    ...
    <Transition name="slide-panel">
      <div v-if="panelVisible" class="fixed inset-0 z-50 flex">
        ...
        <div
          :class="
            dark ? 'bg-slate-900 border-l border-slate-700' : 'bg-white border-l border-zinc-200'
          "
        ...
        >
          ...
          >
            <div class="flex items-center gap-3">
              ...
              <div>
                <h3 :class="dark ? 'text-white' : 'text-zinc-800'" class="text-lg font-semibold">
                  {{
                    panelMode === 'view'
                      ? 'Détail entité'
                      : panelMode === 'create'
                ...
                <p :class="dark ? 'text-slate-500' : 'text-zinc-400'" class="text-xs mt-0.5">
          ...
          <div v-if="panelMode === 'view' && panelEntity" class="flex-1 overflow-y-auto px-6 py-5">
            ...
            <div class="flex items-center gap-2 mb-5">
              <span
                :class="[
                  entityTypeColorClasses(panelEntity.entity_type).bg,
                  entityTypeColorClasses(panelEntity.entity_type).border,
                  entityTypeColorClasses(panelEntity.entity_type).text,
                ]"
            ...
            <!-- Nom -->
            <h4 :class="dark ? 'text-white' : 'text-zinc-800'" class="text-lg font-bold mb-4">
              {{ panelEntity.name }}
            </h4>
            ...
            <fieldset class="mb-6">
              ...
              <dl class="space-y-2.5">
                ...
                <div v-else-if="panelEntity.parent_id && getInheritedSiret(panelEntity.parent_id)">
                  ...
                  <dd :class="dark ? 'text-white/60' : 'text-zinc-500'" class="text-sm font-mono">
              ...
              <dl class="space-y-2.5">
                ...
                <div v-if="panelEntity.email">
                  ...
                  <dd :class="dark ? 'text-white' : 'text-zinc-700'" class="text-sm">
                    <i
                      ...
                      class="pi pi-envelope text-xs mr-1.5"
                    ></i
                    >{{ panelEntity.email }}
                  </dd>
                </div>
                <div v-if="panelEntity.phone">
                  ...
                  <dd :class="dark ? 'text-white' : 'text-zinc-700'" class="text-sm">
                    <i
                      :class="dark ? 'text-slate-500' : 'text-zinc-400'"
                      class="pi pi-phone text-xs mr-1.5"
                    ></i
                    >{{ panelEntity.phone }}
                  </dd>
                </div>
                <div
                  ...
                  "
                >
                  <p :class="dark ? 'text-slate-600' : 'text-zinc-400'" class="text-sm italic">
              ...
              <dl class="space-y-2.5">
                <div v-if="viewParentEntity">
                  ...
                  <dd :class="dark ? 'text-white' : 'text-zinc-700'" class="text-sm">
                    <i
                      :class="dark ? 'text-slate-500' : 'text-zinc-400'"
                      class="pi pi-arrow-up text-xs mr-1.5"
                    ></i>
                    {{ viewParentEntity.name }}
                    <span :class="dark ? 'text-slate-500' : 'text-zinc-400'" class="text-xs ml-1">
            ...
            <fieldset>
              ...
              <div class="grid grid-cols-2 gap-3">
                ...
                >
                  <p
                    :class="dark ? 'text-sky-400' : 'text-sky-600'"
                    class="text-lg font-bold tabular-nums"
                  >
                    {{ panelEntity?.users_count || 0 }}
                  </p>
                  <p :class="dark ? 'text-slate-500' : 'text-zinc-400'" class="text-[11px]">
                    Professionnels
                  </p>
          ...
          >
            ...
            >
              <span>
                ...
                <strong>{{
                  entities.find((e) => e.id === panelParentId)?.name || rootEntity?.name
                }}</strong>
              </span>
            </Message>
            ...
            <div v-if="panelMode === 'create'" class="mb-6">
              ...
              >
                ...
                <p :class="dark ? 'text-slate-400' : 'text-zinc-500'" class="text-xs mb-3">
                      ...
                      >
                        <div class="flex items-start justify-between gap-3">
                          ...
                          <span
                            ...
                            :class="
                              dark ? 'bg-slate-700 text-slate-400' : 'bg-zinc-100 text-zinc-400'
                            "
            ...
            <fieldset class="mb-6">
              ...
              <div class="space-y-4">
                ...
                <div
                  v-if="panelMode === 'edit' && panelEntity && !panelEntity.parent_id && tenantCode"
                >
                  <label :class="labelClass" class="block text-sm font-medium mb-1.5">
                    Code structure
                  </label>
                  <div
                    :class="
                  ...
                  >
                    <i
                      :class="dark ? 'text-amber-400' : 'text-amber-500'"
                      class="pi pi-key text-xs"
                    ></i>
                    {{ tenantCode }}
                    <span
                ...
                <div class="grid grid-cols-2 gap-3">
                  <div>
                    <label :class="labelSmallClass" class="block text-xs font-medium mb-1.5">
                      FINESS Juridique (EJ)
                    </label>
                    <InputText
              ...
              <div class="space-y-4">
                ...
                <div class="grid grid-cols-3 gap-3">
                  <div>
                    ...
                    <InputText v-model="form.postal_code" :class="inputClass" placeholder="75000" />
                  </div>
                  <div class="col-span-2">
                    <label :class="labelSmallClass" class="block text-xs font-medium mb-1.5">
                      Ville
                    </label>
                    <InputText v-model="form.city" :class="inputClass" placeholder="Paris" />
                  </div>
                </div>
          ...
          >
            ...
            <template v-else>
              <Button label="Annuler" severity="secondary" size="small" text @click="closePanel" />
              <Button
      ...
      <div class="flex items-start gap-3 py-2">
        ...
        <div>
          ...
          <template v-if="deleteDescendantsCount > 0">
            <div class="mt-3 p-3 rounded-lg bg-red-50 border border-red-200">
              ...
              <p class="text-xs text-red-600">
                ...
                <strong
                  >{{ deleteDescendantsCount }} entité{{
                    deleteDescendantsCount > 1 ? 's' : ''
                  }}</strong
          ...
          <p v-else class="text-xs text-zinc-400 mt-1.5">
            Cette entité sera retirée de l'arborescence. Les données associées (patients,
            professionnels) devront être réaffectées.
          </p>
        </div>
      </div>
    ...
    >
      <div class="flex items-start gap-3 py-2">
        <div
          class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 bg-amber-100"
        >
      ...
      <template #footer>
        <div class="flex items-center justify-end gap-2">
          <Button label="Plus tard" severity="secondary" size="small" text @click="cancelOrphans" />
          <Button label="Rattacher" icon="pi pi-link" size="small" @click="confirmOrphans" />
        </div>
      </template>
    </Dialog>
  </div>
