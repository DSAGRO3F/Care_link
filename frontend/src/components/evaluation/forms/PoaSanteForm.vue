<!--
  CareLink - PoaSanteForm
  Chemin : frontend/src/components/evaluation/forms/PoaSanteForm.vue

  Rôle : Formulaire de saisie du Plan d'Actions Santé (POA Santé).
         Structure : tableau de problèmes (`problemes[]`), chacun avec un sous-tableau
         d'actions (`planActions[]`). 10 blocs fixes issus du Bachelard :
         Cardio-Respiratoire, Nutrition, Elimination, Mobilité, Peau,
         Sensoriel, Douleur, Cognition, Polymédications,
         Appareillages et Dispositifs Médicaux.
         Émet @update:data et @update:status vers le wizard parent.

  Identité visuelle : fieldset red, classes wizard-* de main.css
  Composant partagé : PoaProblemeCard.vue (même que POA Social)
-->
<template>
  <div class="space-y-7">
    <!-- ── Fieldset POA Santé ───────────────────────────────────────── -->
    <fieldset class="form-fieldset form-fieldset--red">
      <legend class="form-legend">
        <span class="form-legend-icon form-legend-icon--red">
          <Pill :size="16" :stroke-width="2" />
        </span>
      ...
      <!-- État vide (ne devrait pas arriver avec les 10 blocs fixes) -->
      <div
        v-if="problemes.length === 0"
        class="flex flex-col items-center py-8 text-slate-400 text-sm"
      >
        <Pill :size="32" :stroke-width="1.2" class="mb-2 text-slate-300" />
...
<script setup lang="ts">
  ...
  // ── Référentiel local (propre au POA Santé) ──────────────────────────

  /**
   * Les 10 blocs du POA Santé (ordre Bachelard).
  ...
  onMounted(() => {
    ...
    const validBlocs = new Set<string>(BLOCS_SANTE);
    ...
    if (source?.problemes && Array.isArray(source.problemes)) {
      ...
      // Aucune donnée : créer les 10 blocs vides
      problemes.value = BLOCS_SANTE.map((bloc) => createEmptyProbleme(bloc));
    }
