<script setup lang="ts">
  ...
  import type { UserSummary, RoleResponse } from '@/types';
  ...
  const avatarClasses = computed(() => {
    ...
    const color = props.user.is_active ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-400';
    return `${base} ${size} ${color}`;
  });

  const cardClasses = computed(() => {
    const base =
      'flex items-center gap-3 bg-white border border-slate-200 rounded-xl shadow-sm transition-all';
...
<template>
  <div :class="cardClasses" @click="handleClick">
    ...
    <div class="flex-1 min-w-0">
      <!-- Nom -->
      <p :class="compact ? 'text-sm' : ''" class="font-medium text-slate-800 truncate">
        {{ fullName }}
      </p>

      <!-- Email -->
      <p v-if="showEmail" :class="compact ? 'text-xs' : 'text-sm'" class="text-slate-400 truncate">
        {{ user.email }}
      </p>
