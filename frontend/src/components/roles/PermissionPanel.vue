...
function getCategoryColors(color: string) {
  const map: Record<string, { bg: string; text: string; check: string; iconBg: string }> = {
    teal:   { bg: 'bg-teal-50/50',   text: 'text-teal-700',   check: 'text-teal-500',   iconBg: 'bg-teal-100 text-teal-600' },
    blue:   { bg: 'bg-blue-50/50',   text: 'text-blue-700',   check: 'text-blue-500',   iconBg: 'bg-blue-100 text-blue-600' },
    red:    { bg: 'bg-red-50/50',    text: 'text-red-700',    check: 'text-red-500',    iconBg: 'bg-red-100 text-red-600' },
    violet: { bg: 'bg-violet-50/50', text: 'text-violet-700', check: 'text-violet-500', iconBg: 'bg-violet-100 text-violet-600' },
    amber:  { bg: 'bg-amber-50/50',  text: 'text-amber-700',  check: 'text-amber-500',  iconBg: 'bg-amber-100 text-amber-600' },
    slate:  { bg: 'bg-slate-50',     text: 'text-slate-700',  check: 'text-slate-500',  iconBg: 'bg-slate-100 text-slate-600' },
    zinc:   { bg: 'bg-zinc-50',      text: 'text-zinc-700',   check: 'text-zinc-500',   iconBg: 'bg-zinc-100 text-zinc-600' },
  }
      ...
      <div class="flex items-start gap-4 p-5 rounded-xl bg-amber-50/60 border border-amber-200">
        ...
        <div>
          <p class="font-semibold text-amber-800 mb-1">
            Accès complet — ADMIN_FULL
          </p>
          <p class="text-sm text-amber-700 leading-relaxed">
            Ce rôle court-circuite toutes les vérifications de permission.
            Un utilisateur avec le rôle <strong>ADMIN</strong> dispose d'un accès
            total à l'ensemble des fonctionnalités du tenant, indépendamment
        ...
        >
          ...
          >
            <i
              ...
              :class="getCategoryColors(group.color).check"
            />
