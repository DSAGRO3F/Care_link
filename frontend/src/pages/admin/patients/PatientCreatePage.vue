<script setup lang="ts">
  ...
  const isSaving = computed(() => patientStore.isSaving);
  ...
  async function submit() {
    ...
    // Exclure les champs adresse locaux du spread (noms form ≠ noms API)
    const { addressLine: _al, addressPostalCode: _ap, addressCity: _ac, ...formRest } = form.value;

    const payload: PatientCreate = {
      ...formRest,
      nir: form.value.nir || undefined,
      ins: form.value.ins || undefined,
      address: form.value.addressLine || undefined,
      postal_code: form.value.addressPostalCode || undefined,
      city: form.value.addressCity || undefined,
    ...
    if (result) {
      ...
      toast.add({
        ...
        detail:
          patientStore.error || 'Impossible de créer le patient. Vérifiez les données saisies.',
        life: 6000,
      });
    }
  }
...
<template>
  <div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
      <Button icon="pi pi-arrow-left" severity="secondary" variant="text" rounded @click="cancel" />
      <div>
    ...
    <div class="flex items-center justify-center gap-2">
      <template v-for="step in stepLabels" :key="step.number">
        ...
        <i v-if="step.number < totalSteps" class="pi pi-chevron-right text-slate-300 text-xs" />
      </template>
    </div>

    <!-- ─── ERREUR GLOBALE ─── -->
    <Message v-if="patientStore.error" :closable="false" severity="error">
      {{ patientStore.error }}
    </Message>
      ...
      <template #content>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="flex flex-col gap-1">
            ...
            <InputText v-model="form.nir" placeholder="1 85 01 75 108 042 36" maxlength="15" />
          </div>
          ...
          <div class="flex flex-col gap-1">
            ...
            <InputText v-model="form.ins" placeholder="Identifiant INS" />
          </div>
        </div>
      </template>
    </Card>
      ...
      <template #content>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="flex flex-col gap-1 col-span-full">
            <label class="text-sm font-medium text-slate-700">N° et nom de rue</label>
            <InputText v-model="form.addressLine" placeholder="12 avenue Caffin" />
          </div>

          <!-- Code postal + Ville -->
          <div class="flex flex-col gap-1">
            <label class="text-sm font-medium text-slate-700">Code postal</label>
            <InputText v-model="form.addressPostalCode" placeholder="94100" maxlength="5" />
          </div>

          <div class="flex flex-col gap-1">
            <label class="text-sm font-medium text-slate-700">Ville</label>
            <InputText v-model="form.addressCity" placeholder="La Varenne Saint-Hilaire" />
          </div>

          <!-- Téléphone -->
          <div class="flex flex-col gap-1">
            <label class="text-sm font-medium text-slate-700">Téléphone</label>
            <InputText v-model="form.phone" placeholder="06 12 34 56 78" />
          </div>

          <!-- Email -->
          <div class="flex flex-col gap-1">
            <label class="text-sm font-medium text-slate-700">Email</label>
            <InputText v-model="form.email" type="email" placeholder="patient@email.com" />
          </div>
        </div>
        ...
        <div class="mt-4 p-3 bg-slate-50 rounded-lg">
          <p class="text-sm text-slate-500">
            ...
            Toutes les données personnelles sont chiffrées (AES-256-GCM) conformément aux exigences
            RGPD/HDS.
          </p>
        </div>
      </template>
    ...
    <Card v-if="currentStep === 3">
      ...
      <template #content>
        <div class="space-y-4">
          ...
          <div>
            <label class="text-sm font-medium text-slate-700 mb-2 block"> Médecin traitant </label>
    ...
    <div class="flex items-center justify-between pt-2">
      ...
      <div class="flex gap-2">
        <Button label="Annuler" severity="secondary" variant="text" @click="cancel" />
        <Button
          v-if="!isLastStep"
          :disabled="!canProceed"
          label="Suivant"
