<script setup lang="ts">
  ...
  onMounted(() => {
    ...
    Promise.allSettled([
      fetchCount(0, '/users?page=1&size=1', (d) => Number(d.total)),
      fetchCount(1, '/patients?page=1&size=1', (d) => Number(d.total)),
      fetchCount(2, '/organizations/entities?page=1&size=1', extractTotal),
...
<template>
  <div class="space-y-8">
    <!-- ── Header de bienvenue ── -->
    <div>
      <h1 class="text-2xl font-semibold text-slate-800">Administration</h1>
      <p class="mt-1 text-sm text-slate-500">Vue d'ensemble de votre structure</p>
    </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
      ...
      <div class="px-6 py-12 text-center">
        ...
        <p class="text-sm text-slate-400">Le journal d'activité sera disponible prochainement</p>
      </div>
    </div>
  </div>
</template>
