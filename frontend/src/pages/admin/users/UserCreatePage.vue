...
import { userService, getErrorMessage } from '@/services/user.service'
import { entityService } from '@/services/entity.service'
import type { UserCreate, UserEntityCreate, RoleResponse, ProfessionResponse } from '@/types/user'
...
const contractType = ref<string>('SALARIE')
...
const professionGroups = computed<ProfessionGroup[]>(() => {
  return PROFESSION_CATEGORY_ORDER
    .map(cat => ({
      category: cat,
...
async function handleSubmit() {
  ...
  try {
    ...
    if (selectedEntityId.value) {
      ...
      try {
        const entityData: UserEntityCreate = {
          ...
          is_primary: true,
          contract_type: contractType.value as any,
    ...
    if (import.meta.env.DEV) {
      console.error('[Submit] Erreur globale:', error.response?.data || error)
    }
    toast.add({
      severity: 'error',
      summary: 'Erreur de création',
      detail: error.response?.data?.detail || getErrorMessage(error),
      life: 8000,
    })
  } finally {
...
<template>
  <div class="max-w-4xl mx-auto pb-12">
    ...
    <div class="mb-8">
      ...
      <p class="text-slate-500">Créez un compte, rattachez à une entité et assignez des responsabilités.</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div v-show="currentStep === 1" class="p-8">
        ...
        <div v-if="selectedEntity" class="mb-6">
          <div class="flex items-center justify-between bg-blue-50 border border-blue-200 rounded-xl px-5 py-4">
            ...
            <Button label="Changer" icon="pi pi-pencil" severity="secondary" variant="text" size="small" @click="clearEntitySelection" />
          </div>

          <div class="mt-4">
            <label class="block text-sm font-medium text-slate-600 mb-2">Type de contrat</label>
            <Select v-model="contractType" :options="contractTypeOptions" optionLabel="label" optionValue="value" class="w-full md:w-64" />
          </div>
        </div>
        ...
        <div v-else>
          ...
          <div v-else-if="entities.length === 0" class="text-center py-12">
            ...
            <p class="text-sm text-slate-400 mt-1">Créez d'abord des entités dans l'espace Structure.</p>
          </div>
          ...
          <Message severity="info" :closable="false" class="mt-4">
            <template #default>
              <span class="text-sm">Le rattachement est optionnel. Vous pourrez l'ajouter plus tard depuis le profil.</span>
            </template>
          </Message>
        </div>
      </div>
      ...
      <div v-show="currentStep === 2" class="p-8">
        <div class="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p class="text-sm text-slate-400">Responsabilités complémentaires dans CareLink (optionnel)</p>
          </div>
        </div>
        ...
        <div class="mb-8">
          ...
          <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-2">
            ...
            >
              <Checkbox :modelValue="isRoleSelected(role.id)" :binary="true" @click.stop @update:modelValue="toggleRole(role.id)" />
              <div class="flex-1 min-w-0">
                <p class="font-medium text-slate-800 text-sm">{{ role.name }}</p>
                <p v-if="role.description" class="text-xs text-slate-400 truncate">{{ role.description }}</p>
              </div>
              <Tag v-if="role.is_system_role" value="Système" severity="info" class="text-xs shrink-0" />
            </button>
          </div>
        </div>
        ...
        <!-- Récapitulatif -->
        <h3 class="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-4">Récapitulatif</h3>

        <div class="bg-slate-50 rounded-xl p-6 space-y-4">
          <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
            <div>
              <p class="text-xs text-slate-400">Nom complet</p>
              <p class="font-medium text-slate-800">{{ identityForm.last_name }} {{ identityForm.first_name }}</p>
            </div>
            <div>
          ...
          <div>
            ...
            <p v-if="selectedEntity" class="text-slate-700">
              ...
              — contrat {{ contractType.toLowerCase() }}
            </p>
          ...
          <div>
            ...
            <div v-if="getSelectedRoles().length > 0" class="flex flex-wrap gap-1.5">
              <Tag v-for="role in getSelectedRoles()" :key="role.id" :value="role.name" severity="info" />
            </div>
      ...
      <!-- FOOTER NAVIGATION -->
      <div class="flex items-center justify-between px-8 py-4 bg-slate-50 border-t border-slate-200">
        <Button label="Annuler" severity="secondary" variant="text" @click="handleCancel" />
        <div class="flex items-center gap-3">
          <Button v-if="currentStep > 0" label="Précédent" severity="secondary" variant="outlined" icon="pi pi-arrow-left" @click="prevStep" />
          <Button v-if="currentStep < 2" label="Suivant" icon="pi pi-arrow-right" iconPos="right" :disabled="!canProceed" @click="nextStep" />
          <Button v-if="currentStep === 2" label="Créer le professionnel" icon="pi pi-check" severity="success" :loading="isSubmitting" :disabled="isSubmitting" @click="handleSubmit" />
        </div>
      </div>
    </div>
  </div>
